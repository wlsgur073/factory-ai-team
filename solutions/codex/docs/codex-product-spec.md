...
└── 관리 (시스템관리자 + 스튜어드)
├── 공통코드 관리 (/admin/common-codes) [관리자/스튜어드]
├── 사용자 관리 (/admin/users)
...
6. ~~DATA_STEWARD 거버넌스 API 접근~~ → 모든 거버넌스 API에 거버넌스 권한 적용
...
10. ~~DATA_STEWARD 대시보드 라우팅~~ → isApproverView 플래그 추가
11. ~~권한 관리 탭~~ → 5개 역할(시스템관리/검토승인/스튜어드/신청자/조회전용)으로 수정
12. ~~DATA_STEWARD 공통코드 관리 접근~~ → canManageCommonCodes 권한 + 사이드바 분리
