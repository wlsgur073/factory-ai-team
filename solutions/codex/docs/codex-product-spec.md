...
| 데이터스튜어드 | `DATA_STEWARD`      | 데이터 품질 관리, 검증 실행        | 거버넌스 포털, 검증 대시보드 |
...
| API                            | 기능                              | 주요 사용처       |
| ------------------------------ | --------------------------------- | ----------------- |
| `POST /api/ai/suggest`         | 유사 표준 추천 (유사도 점수 포함) | 신규 신청, 탐색기 |
| `POST /api/ai/autocomplete`    | 필드 자동완성 (영문약어, 정의)    | 폼 입력           |
| `POST /api/ai/quality-score`   | 데이터 품질 점수 산출             | 승인 워크벤치     |
| `POST /api/ai/synonyms`        | 유사어/대체 표준 검색             | 삭제 영향도 평가  |
...
interface Draft extends BaseEntity {
  ...
  status: DraftStatus; // EDITING | READY | SUBMITTED | EXPIRED
...
interface Notification {
  ...
  userId: number;
  type: NotificationType; // APPROVAL_REQUIRED | REQUEST_APPROVED | ...
  title: string;
  message: string;
...
- **GOV-004**: 승인 완료 시 원본 데이터에 자동 반영 (표준 상태 → BASELINE)
...
리뷰에서 발견된 5개 Major 이슈는 모두 수정 반영 완료:
